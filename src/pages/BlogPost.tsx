...
export default function BlogPostPage() {
  ...
  // Simple markdown-ish renderer (newlines + headers)
  const renderContent = (content: string) => {
    const lines = content.trim().split("\n");
    return lines.map((line, i) => {
      if (line.startsWith("## "))
        return <h2 key={i} className="text-xl font-bold text-navy mt-7 mb-3">{line.slice(3)}</h2>;
      if (line.startsWith("### "))
        return <h3 key={i} className="text-lg font-bold text-navy mt-5 mb-2">{line.slice(4)}</h3>;
      if (line.startsWith("**") && line.endsWith("**"))
        return <p key={i} className="font-bold text-navy mb-2">{line.slice(2, -2)}</p>;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 pt-24 pb-12 max-w-4xl">
        <button
          ...
          className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-navy mb-6 transition-colors"
        >
          <ArrowLeft className="h-4 w-4" /> Back to Blog
        </button>

        {/* Cover Image */}
        <div className="rounded-2xl overflow-hidden h-64 mb-7">
        ...
        <div className="flex flex-wrap gap-2 mt-8 pt-6 border-t border-border">
          {post.tags.map((tag) => (
            <span key={tag} className="bg-muted text-muted-foreground text-xs px-3 py-1.5 rounded-full">
              #{tag}
            </span>
          ))}
        </div>
        ...
        {/* CTA */}
        <div className="bg-navy text-white rounded-2xl p-6 text-center mt-10">
          <h3 className="font-bold text-lg mb-2">Interested in Investing?</h3>
          <p className="text-white/70 text-sm mb-4">
            Talk to our experts for personalized property advice — completely free.
          </p>
          <button
